    ...
    try:
        ...
        for frame_idx in tqdm(range(total_frames), desc="Pre-rendering shader"):
            ...
            if current_time - last_update_time >= update_interval or frame_idx == total_frames - 1:
                ...
                # Use full 0-100% range for shader pre-rendering
                progress_percent = int(100 * (frame_idx + 1) / total_frames)
        ...
        # Final status update
        update_status(status_file, 100, "Shader pre-rendering complete")
...
def main():
    ...
    if result:
        update_status(args.status_file, 100, f"Shader pre-rendering complete: {result}")
    else:
