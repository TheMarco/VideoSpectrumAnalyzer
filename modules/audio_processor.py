...
def analyze_audio(y, sr, n_bars, min_freq, max_freq, fps, progress_callback=None):
    ...
    threshold_adjustments[:bass_limit] = 1.2  # Bass threshold adjustment
    threshold_adjustments[bass_limit:mid_limit] = 1.0  # Mid threshold adjustment
    threshold_adjustments[mid_limit:] = 0.9  # High threshold adjustment
    
    freq_avg_energy = np.mean(mel_spec_norm, axis=1)
    dynamic_thresholds = freq_avg_energy * 0.3 * threshold_adjustments  # 0.3 is threshold factor
