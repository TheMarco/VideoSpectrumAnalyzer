...
def create_spectrum_analyzer(audio_file,
                            ...
                            background_image_path=None, # Keep image path
                            background_video_path=None, # <-- Add video path
    ...
    default_config = {
        ...
        'always_on_bottom': True, 'peak_hold_frames': 5, 'peak_decay_speed': 0.15,
        ...
        'use_gradient': False, 'gradient_top_color': (200, 200, 255),
    ...
    if config and isinstance(config, dict):
        ...
        for key, value in config.items():
             if key in default_config:
                try:
                    if key in bool_keys:
                        if isinstance(value, str): processed_config[key] = value.lower() in ('true', '1', 'on', 'yes')
                        else: processed_config[key] = bool(value)
                    elif key in float_keys:
                        if key == 'duration' and value is not None and float(value) == 0: processed_config[key] = None
                        elif value is not None: processed_config[key] = float(value)
                        else: processed_config[key] = default_config[key] # Handle None for floats
    ...
    try:
        ...
        print(f"ERROR Loading Audio: {e}"); raise
    if len(y.shape) > 1: y = np.mean(y, axis=1)
    audio_duration = librosa.get_duration(y=y, sr=sr)
    if duration is None or duration <= 0 or duration > audio_duration: duration = audio_duration
    sample_count = int(duration * sr); y = y[:sample_count]
    ...
    video_capture = None
    bg_frame_count = 0
    bg_fps = 30 # Default, will be overwritten
    ...
    if background_video_path and os.path.exists(background_video_path):
        try:
            ...
            if bg_frame_count <= 0 or bg_fps <= 0:
                 print(f"Warning: Could not read properties (frame count/fps) from video {background_video_path}. Looping might be unreliable.")
                 ret, _ = video_capture.read()
                 if not ret: raise IOError(f"Cannot read first frame from video: {background_video_path}")
                 print("Proceeding with video, but properties seem invalid.")
            ...
            print(f"Warning: Could not load or process background video: {e}. Checking for image fallback.")
            if video_capture: video_capture.release()
            video_capture = None
            background_video_path = None # Clear video path if loading failed

    # If video didn't load or wasn't provided, check for image
    if not video_capture and background_image_path and os.path.exists(background_image_path):
        try:
            ...
            with Image.open(background_image_path) as _img:
                background_pil = _img.convert('RGBA') # Ensure RGBA
            ...
            print("Background image loaded, converted to RGBA, cropped, and resized.")
        except Exception as e:
    ...
    if D_filtered.shape[0] == 0: print("Warning: No frequency bins selected after filtering."); D_filtered = np.zeros((1, D.shape[1]))
    if D_filtered.size == 0: raise ValueError("Filtered spectrum is empty.")
    ...
    if max_db <= min_db: mel_spec_norm = np.zeros_like(mel_spec_db); print("Warning: Spectrum has constant value.")
    else: mel_spec_norm = (mel_spec_db - min_db) / (max_db - min_db + 1e-6)
    ...
    if actual_frames == 0: raise ValueError("Audio analysis resulted in 0 frames.")
    ...
    try:
        if os.path.exists(local_font_path_bold) and os.path.exists(local_font_path_regular):
            ...
            font_load_success = True; print("   Success loading local fonts.")
        else: raise IOError("Local fonts not found")
    except Exception as e:
         print(f"   Error loading local fonts: {e}\nAttempting fallback system fonts...")
         for bold_path, regular_path in fallback_font_paths:
             if font_load_success: break
             try:
                 artist_font = ImageFont.truetype(bold_path, font_size_artist); title_font = ImageFont.truetype(regular_path, font_size_title)
                 font_load_success = True; print(f"   Success loading fallback pair: {bold_path}, {regular_path}."); break
             except: pass
    if not font_load_success:
        print("--- All font loading attempts failed. Falling back to default font. ---")
        artist_font = ImageFont.load_default(); title_font = ImageFont.load_default()
    ...
    ffmpeg_cmd = [ 'ffmpeg', '-y', '-f', 'rawvideo', '-vcodec', 'rawvideo', '-s', f'{width}x{height}', '-pix_fmt', 'rgba', '-r', str(fps), '-i', '-', '-an', '-c:v', 'libx264', '-preset', 'fast', '-crf', '23', '-pix_fmt', 'yuv420p', temp_video_path ] # Use RGBA input pipe now
    print("Starting FFmpeg process for video frames...")
    try:
        process = subprocess.Popen(ffmpeg_cmd, stdin=subprocess.PIPE)
    except FileNotFoundError: print("\n"+"="*60+"\n FATAL ERROR: ffmpeg command not found.\n"+"="*60+"\n"); raise
    except Exception as e: print(f"ERROR: Failed to start FFmpeg process: {e}"); raise
    ...
    if seg_width <= 0 or seg_height <= 0: raise ValueError(f"Invalid segment dimensions: {seg_width}x{seg_height}")

    last_good_bg_frame_pil = None # Fallback if video read fails mid-stream
    ...
    for frame_idx in tqdm(range(actual_frames), desc="Generating Frames"):
        ...
        if frame_idx < len(normalized_frame_energy): is_silent = normalized_frame_energy[frame_idx] < SILENCE_THRESHOLD
        else: is_silent = True; print(f"Warning: frame_idx {frame_idx} out of bounds for energy")
        for i in range(N_BARS):
            if is_silent: smoothed_spectrum[i]*=SILENCE_DECAY_FACTOR; peak_values[i]*=SILENCE_DECAY_FACTOR
            else:
                ...
                else: smoothed_spectrum[i] = smoothed_spectrum[i] * (1-DECAY_SPEED)
                if smoothed_spectrum[i] < NOISE_GATE: smoothed_spectrum[i] = 0.0
                if smoothed_spectrum[i] > peak_values[i]: peak_values[i]=smoothed_spectrum[i]; peak_hold_counters[i]=PEAK_HOLD_FRAMES
                elif peak_hold_counters[i] > 0: peak_hold_counters[i] -= 1
                else: peak_values[i]=max(peak_values[i]*(1-PEAK_DECAY_SPEED), smoothed_spectrum[i])
                if peak_values[i] < NOISE_GATE: peak_values[i] = 0.0
        ...
        if video_capture:
            ...
            if not ret: # End of video or read error
                print(f"End of background video reached or read error at frame {frame_idx}. Looping...")
                video_capture.set(cv2.CAP_PROP_POS_FRAMES, 0) # Rewind
                ret, frame_bgr = video_capture.read() # Read the first frame again
                if not ret:
                    print("ERROR: Failed to read frame even after looping video. Using last good frame or solid color.")
                    if last_good_bg_frame_pil:
                        current_bg_frame_pil = last_good_bg_frame_pil.copy()
                    else: # Absolute fallback
                        video_capture.release() # Stop trying
                        video_capture = None
                        current_bg_frame_pil = Image.new('RGBA', (width, height), color=BACKGROUND_COLOR + (255,))
                # else: Successfully looped and read frame

            if ret: # If frame read was successful (either normal or after loop)
                try:
                    frame_rgba = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGBA)
                    # Resize the video frame to match output dimensions
                    ...
                    last_good_bg_frame_pil = current_bg_frame_pil.copy() # Store for fallback
                except Exception as e:
                     print(f"Error converting/resizing video frame {frame_idx}: {e}. Using fallback.")
                     if last_good_bg_frame_pil: current_bg_frame_pil = last_good_bg_frame_pil.copy()
                     else: current_bg_frame_pil = Image.new('RGBA', (width, height), color=BACKGROUND_COLOR + (255,))

        elif background_pil: # Use the static PIL image
        ...
        # ================================================================
        for i in range(N_BARS):
            bar_x = start_x + i * total_bar_width_gap; signal = smoothed_spectrum[i]; peak_signal = peak_values[i]
            if ALWAYS_ON_BOTTOM_FLAG and max_segments >= 1:
                ...
                static_color_rgb = GRADIENT_BOTTOM_COLOR if USE_GRADIENT else BAR_COLOR_RGB; static_color_rgba = static_color_rgb + (PIL_ALPHA,)
                ...
                image.alpha_composite(static_seg_img, static_dest_xy) # Composite ONTO background
            ...
            for k in range(num_dynamic_segments_to_draw):
                ...
                if USE_GRADIENT:
                    gradient_factor_raw = j / max(1, max_segments - 1); gradient_factor_raw = min(1.0, max(0.0, gradient_factor_raw))
                    gradient_factor = gradient_factor_raw ** GRADIENT_EXPONENT
                    r = int(GRADIENT_BOTTOM_COLOR[0]*(1-gradient_factor) + GRADIENT_TOP_COLOR[0]*gradient_factor); g = int(GRADIENT_BOTTOM_COLOR[1]*(1-gradient_factor) + GRADIENT_TOP_COLOR[1]*gradient_factor); b = int(GRADIENT_BOTTOM_COLOR[2]*(1-gradient_factor) + GRADIENT_TOP_COLOR[2]*gradient_factor)
            ...
            if peak_signal > NOISE_GATE: peak_total_segments_float = peak_signal * max_segments * EFFECTIVE_AMPLITUDE_SCALE; peak_segment_index_abs = min(int(np.ceil(peak_total_segments_float)), max_segments) - 1
            ...
            if peak_segment_index_abs >= 0 and peak_segment_index_abs > highest_dynamic_segment_index_abs:
                if not (ALWAYS_ON_BOTTOM_FLAG and peak_segment_index_abs == 0):
                    ...
                    if USE_GRADIENT:
                        gradient_factor_raw = peak_j / max(1, max_segments - 1); gradient_factor_raw = min(1.0, max(0.0, gradient_factor_raw))
                        gradient_factor = gradient_factor_raw ** GRADIENT_EXPONENT
                        r=int(GRADIENT_BOTTOM_COLOR[0]*(1-gradient_factor)+GRADIENT_TOP_COLOR[0]*gradient_factor); g=int(GRADIENT_BOTTOM_COLOR[1]*(1-gradient_factor)+GRADIENT_TOP_COLOR[1]*gradient_factor); b=int(GRADIENT_BOTTOM_COLOR[2]*(1-gradient_factor)+GRADIENT_TOP_COLOR[2]*gradient_factor)
                    ...
                    image.alpha_composite(peak_img, peak_dest_xy) # Composite ONTO background
        # ================================================================
        ...
        if artist_font and title_font:
            try:
                ...
                if artist_font == ImageFont.load_default() or title_font == ImageFont.load_default(): anchor_artist = "mm"; anchor_title = "mm"
                ...
                if artist_name: draw_text.text((artist_text_x, artist_text_y), artist_name, fill=ARTIST_COLOR_RGB + (255,), font=artist_font, anchor=anchor_artist)
                if track_title: draw_text.text((title_text_x, title_text_y), track_title, fill=TITLE_COLOR_RGB + (255,), font=title_font, anchor=anchor_title)
            except Exception as text_err: print(f"Warning: Error drawing text on frame {frame_idx}: {text_err}")

        # --- Pipe Frame to FFmpeg ---
        try:
            frame_bytes = image.tobytes() # image is already RGBA
            process.stdin.write(frame_bytes)
        except (BrokenPipeError, OSError) as e:
            print(f"\nError writing frame {frame_idx} to FFmpeg: {e}")
            try: process.stdin.close()
            except OSError: pass
            process.wait(); print(f"FFmpeg exit code (pipe error): {process.returncode}")
            # ****** FIX IS HERE ******
            if os.path.exists(temp_video_path):
                try: os.remove(temp_video_path)
                except OSError as rem_e: print(f"Warn: Could not remove temp file: {rem_e}")
            # ****** END FIX ******
            raise RuntimeError(f"FFmpeg pipe error: {e}") from e
             ...
             if process.stdin:
                 try: process.stdin.close()
                 except Exception: pass # Ignore errors closing already broken pipe
             process.poll();
             if process.returncode is None: process.terminate(); process.wait(timeout=5)
             # ****** FIX IS HERE ******
             if os.path.exists(temp_video_path):
                 try: os.remove(temp_video_path)
    ...
    if process.stdin:
        try: process.stdin.close()
        except (OSError, BrokenPipeError) as e: print(f"Warning: Error closing ffmpeg stdin: {e}")
    process.wait()
    if process.returncode != 0:
        print(f"ERROR: FFmpeg video encoding failed (code {process.returncode})")
        if os.path.exists(temp_video_path): os.remove(temp_video_path)
        raise subprocess.CalledProcessError(process.returncode, ffmpeg_cmd)
    else: print("FFmpeg video encoding successful.")
    ...
    try:
        ...
        if result.stderr: print(f"FFmpeg warnings (audio merge):\n{result.stderr}")
    ...
    # --- Cleanup ---
    print("Cleaning up...")
    if video_capture: # Release video capture object
        print("Releasing video capture...")
        video_capture.release()
    if os.path.exists(temp_video_path):
        try: os.remove(temp_video_path); print(f"Removed temp file: {temp_video_path}")
