...
def create_spectrum_analyzer(
    ...
    if config and isinstance(config, dict):
        ...
        float_keys = [
            ...
            "decay_speed",
            "peak_decay_speed",
            "bass_threshold_adjust",
            "mid_threshold_adjust",
            "high_threshold_adjust",
            "silence_threshold",
            "silence_decay_factor",
            "noise_gate",
            "gradient_exponent",
            "duration",
        ]
                ...
                try:
                    if key in bool_keys:
                        if isinstance(value, str):
                            processed_config[key] = value.lower() in (
                                "true",
                                "1",
                                "on",
                                "yes",
                        ...
                        if (
                            ...
                            processed_config[key] = default_config[
                                key
                            ]  # Handle None for floats
    ...
    AMPLITUDE_SCALE = conf["amplitude_scale"]
    SENSITIVITY = conf["sensitivity"]
    ANALYZER_ALPHA = max(0.0, min(1.0, conf["analyzer_alpha"]))
    SEGMENT_HEIGHT = conf["segment_height"]
    SEGMENT_GAP = conf["segment_gap"]
    CORNER_RADIUS = conf["corner_radius"]
    MIN_FREQ = conf["min_freq"]
    MAX_FREQ = conf["max_freq"]
    THRESHOLD_FACTOR = conf["threshold_factor"]
    ATTACK_SPEED = conf["attack_speed"]
    DECAY_SPEED = conf["decay_speed"]
    ALWAYS_ON_BOTTOM_FLAG = conf.get("always_on_bottom", True)
    PEAK_HOLD_FRAMES = conf["peak_hold_frames"]
    PEAK_DECAY_SPEED = conf["peak_decay_speed"]
    BASS_THRESHOLD_ADJUST = conf["bass_threshold_adjust"]
    MID_THRESHOLD_ADJUST = conf["mid_threshold_adjust"]
    HIGH_THRESHOLD_ADJUST = conf["high_threshold_adjust"]
    SILENCE_THRESHOLD = conf["silence_threshold"]
    SILENCE_DECAY_FACTOR = conf["silence_decay_factor"]
    NOISE_GATE = conf["noise_gate"]
    USE_GRADIENT = conf.get("use_gradient", False)
    GRADIENT_TOP_COLOR = (
        tuple(conf["gradient_top_color"])
        if isinstance(conf["gradient_top_color"], (list, tuple))
        else (200, 200, 255)
    )
    GRADIENT_BOTTOM_COLOR = (
        tuple(conf["gradient_bottom_color"])
        if isinstance(conf["gradient_bottom_color"], (list, tuple))
        else (255, 255, 255)
    )
    ...
    if background_video_path and os.path.exists(background_video_path):
        try:
            ...
            if bg_frame_count <= 0 or bg_fps <= 0:
                print(
                    f"Warning: Could not read properties (frame count/fps) from video {background_video_path}. Looping might be unreliable."
                )
                ret, _ = video_capture.read()
                if not ret:
                    raise IOError(
                        f"Cannot read first frame from video: {background_video_path}"
                    )
                print("Proceeding with video, but properties seem invalid.")

            print(
                f"Background video loaded: {bg_width}x{bg_height} @ {bg_fps:.2f} FPS, {bg_frame_count} frames"
            )
            background_pil = None  # Ensure PIL image isn't used

        except Exception as e:
            print(
                f"Warning: Could not load or process background video: {e}. Checking for image fallback."
            )
            if video_capture:
    ...
    if (
        ...
        and os.path.exists(background_image_path)
    ):
        try:
            ...
            if img_ratio > vid_ratio:
                ...
                background_pil = background_pil.crop(
                    (left, 0, left + new_width, background_pil.height)
                )
            elif img_ratio < vid_ratio:
                new_height = int(background_pil.width / vid_ratio)
                top = (background_pil.height - new_height) / 2
                background_pil = background_pil.crop(
                    (0, top, background_pil.width, top + new_height)
                )
            background_pil = background_pil.resize(
                (width, height), Image.Resampling.LANCZOS
            )
            print("Background image loaded, converted to RGBA, cropped, and resized.")
        except Exception as e:
            print(
                f"Warning: Could not load or process background image: {e}. Using solid color."
            )
            background_pil = None
    ...
    energy_95th_percentile = (
        np.percentile(frame_energy, 95) if frame_energy.size > 0 else 1.0
    )
    ...
    try:
        if os.path.exists(local_font_path_bold) and os.path.exists(
            local_font_path_regular
        ):
    ...
    corner_radius = (
        min(CORNER_RADIUS, SEGMENT_HEIGHT // 2, BAR_WIDTH // 2)
        if CORNER_RADIUS > 0
        else 0
    )
    ...
    for frame_idx in tqdm(range(actual_frames), desc="Generating Frames"):
        ...
        if frame_idx < len(normalized_frame_energy):
            ...
            is_silent = True
            print(f"Warning: frame_idx {frame_idx} out of bounds for energy")
        for i in range(N_BARS):
            if is_silent:
                ...
                if current_spectrum[i] > dynamic_thresholds[i]:
                    strength = np.clip(
                        (current_spectrum[i] - dynamic_thresholds[i])
                        / (1 - dynamic_thresholds[i] + 1e-6),
                        0,
                        1,
                    )
                ...
                if smoothed_spectrum[i] > peak_values[i]:
                    ...
                    peak_values[i] = max(
                        peak_values[i] * (1 - PEAK_DECAY_SPEED), smoothed_spectrum[i]
                    )
        ...
        if video_capture:
            ...
            if not ret:  # End of video or read error
                print(
                    f"End of background video reached or read error at frame {frame_idx}. Looping..."
                )
                video_capture.set(cv2.CAP_PROP_POS_FRAMES, 0)  # Rewind
                ret, frame_bgr = video_capture.read()  # Read the first frame again
                if not ret:
                    print(
                        "ERROR: Failed to read frame even after looping video. Using last good frame or solid color."
                    )
                    if last_good_bg_frame_pil:
                        ...
                        video_capture = None
                        current_bg_frame_pil = Image.new(
                            "RGBA", (width, height), color=BACKGROUND_COLOR + (255,)
                        )
            ...
            if ret:  # If frame read was successful (either normal or after loop)
                try:
                    ...
                    interp = (
                        cv2.INTER_AREA
                        if frame_rgba.shape[1] > width or frame_rgba.shape[0] > height
                        else cv2.INTER_LANCZOS4
                    )
                    resized_frame = cv2.resize(
                        frame_rgba, (width, height), interpolation=interp
                    )
                    current_bg_frame_pil = Image.fromarray(resized_frame, "RGBA")
                    last_good_bg_frame_pil = (
                        current_bg_frame_pil.copy()
                    )  # Store for fallback
                except Exception as e:
                    print(
                        f"Error converting/resizing video frame {frame_idx}: {e}. Using fallback."
                    )
                    if last_good_bg_frame_pil:
                        current_bg_frame_pil = last_good_bg_frame_pil.copy()
                    else:
        ...
        for i in range(N_BARS):
            ...
            if ALWAYS_ON_BOTTOM_FLAG and max_segments >= 1:
                ...
                static_seg_img = Image.new(
                    "RGBA", (seg_width, seg_height), (0, 0, 0, 0)
                )
                static_seg_draw = ImageDraw.Draw(static_seg_img)
                if corner_radius == 0:
                    static_seg_draw.rectangle(
                        (0, 0, seg_width, seg_height), fill=static_color_rgba
                    )
                else:
            ...
            if signal > NOISE_GATE and num_segments_available_above > 0:
                dynamic_segments_float = (
                    signal * num_segments_available_above * EFFECTIVE_AMPLITUDE_SCALE
                )
            ...
            for k in range(num_dynamic_segments_to_draw):
                ...
                if USE_GRADIENT:
                    ...
                    r = int(
                        GRADIENT_BOTTOM_COLOR[0] * (1 - gradient_factor)
                        + GRADIENT_TOP_COLOR[0] * gradient_factor
                    )
                    g = int(
                        GRADIENT_BOTTOM_COLOR[1] * (1 - gradient_factor)
                        + GRADIENT_TOP_COLOR[1] * gradient_factor
                    )
                    b = int(
                        GRADIENT_BOTTOM_COLOR[2] * (1 - gradient_factor)
                        + GRADIENT_TOP_COLOR[2] * gradient_factor
                    )
                    segment_color_rgb = (
                        max(0, min(r, 255)),
                        max(0, min(g, 255)),
                ...
                if corner_radius == 0:
                    segment_draw_alpha.rectangle(
                        (0, 0, seg_width, seg_height), fill=segment_color_rgba
                    )
                else:
                    segment_draw_alpha.rounded_rectangle(
                        (0, 0, seg_width, seg_height),
                        radius=corner_radius,
            ...
            if peak_signal > NOISE_GATE:
                peak_total_segments_float = (
                    peak_signal * max_segments * EFFECTIVE_AMPLITUDE_SCALE
                )
                peak_segment_index_abs = (
                    min(int(np.ceil(peak_total_segments_float)), max_segments) - 1
                )
            highest_dynamic_segment_index_abs = (
                (num_dynamic_segments_to_draw - 1)
                + (
                    1
                    if ALWAYS_ON_BOTTOM_FLAG and num_dynamic_segments_to_draw > 0
                    else 0
                )
                if num_dynamic_segments_to_draw > 0
                else -1
            )
            if (
                peak_segment_index_abs >= 0
                and peak_segment_index_abs > highest_dynamic_segment_index_abs
            ):
                if not (ALWAYS_ON_BOTTOM_FLAG and peak_segment_index_abs == 0):
                    peak_j = peak_segment_index_abs
                    peak_y = (
                        viz_bottom
                        - (peak_j + 1) * SEGMENT_HEIGHT
                        - peak_j * SEGMENT_GAP
                    )
                    ...
                    if corner_radius == 0:
                        peak_draw.rectangle(
                            (0, 0, seg_width, seg_height), fill=peak_color_rgba
                        )
                    else:
                        peak_draw.rounded_rectangle(
                            (0, 0, seg_width, seg_height),
                            radius=corner_radius,
        ...
        if artist_font and title_font:
            try:
                ...
                if (
                    artist_font == ImageFont.load_default()
                    or title_font == ImageFont.load_default()
                ):
                    anchor_artist = "mm"
                    anchor_title = "mm"
                artist_bbox = (
                    draw_text.textbbox((0, 0), artist_name, font=artist_font)
                    if artist_name
                    else (0, 0, 0, 0)
                )
                title_bbox = (
                    draw_text.textbbox((0, 0), track_title, font=title_font)
                    if track_title
                    else (0, 0, 0, 0)
                )
                ...
                title_text_y = (
                    artist_text_y
                    + artist_text_height // 2
                    + 25
                    + (title_bbox[3] - title_bbox[1]) // 2
                )
                if artist_name:
                    draw_text.text(
                        (artist_text_x, artist_text_y),
                        artist_name,
                        fill=ARTIST_COLOR_RGB + (255,),
                        font=artist_font,
        ...
        try:
            ...
            if process.stdin:
                try:
                    ...
                    pass  # Ignore errors closing already broken pipe
            process.poll()
