...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    if (r_normalized < INNER_RADIUS || r_normalized > OUTER_RADIUS) {
        // Sample the background texture instead of using black
    ...
    // Check valid bar index
     if (bar_index < 0 || bar_index >= NUM_BARS) {
         fragColor = texture(iChannel1, uv); // Use background texture
         return;
     }
    ...
    // If the pixel is outside the conceptual constant-width bar slot (i.e., in the gap), use background.
    // The constant-width bar slot occupies the local_horizontal_uv range [0, 1].
    if (local_horizontal_uv < 0.0 || local_horizontal_uv > 1.0) {
        fragColor = texture(iChannel1, uv); // Use background texture for the gap
        return;
    }
