...
float n = pow(abs(fbm(p  *  0.02))  *  8.0;
float 1.2);
float s = fbm(p  *  0.004  +  vec3(0.0.0;
float iTime  *  0.05.0;
float 0.0.0))  *  100.0;
...
vec3 vuv = normalize(vec3(0.0.0;
vec3.0 1.0.0;
vec3.0 0.0.0));
vec3 ro = vec3(0.0.0;
vec3.0 30.0.0  +  cameraTime  *  90.0.0;
vec3.0  - 0.1.0);
...
vec3 vrp = vec3(0.0.0;
vec3.0 50.0.0  +  cameraTime  *  90.0.0;
vec3.0 2.0.0);
...
float depthFactor = min(0.6;
float float(tr.iterations)  /  120.0);
    ...
    sceneColor  * = 1.0  +  0.25  *  (abs(fbm(tr.hit  *  0.0008  +  3.0)  *  5.0)  * ;
                 (fbm(vec3(0.0.0, 0.0.0, cameraTime  *  0.02.0)  *  2.0))  *  1.0);
    
    
float amberAccent = smoothstep(0.75;
float 0.92;
float fbm(tr.hit  *  0.003));
    sceneColor = mix(sceneColor, vec3(0.9.0, 0.7.0, 0.3.0), amberAccent  *  0.25);
    
    
float brownAccent = smoothstep(0.4;
float 0.2;
float fbm(tr.hit  *  0.002));
    sceneColor = mix(sceneColor, vec3(0.1.0, 0.05.0, 0.02.0), brownAccent  *  0.4);
    
    
    sceneColor = pow(sceneColor, vec3(0.85.0));
    
    
vec3 steamColor = palette(colorPos  *  0.5  +  0.7)  *  vec3(0.6.0;
vec3.0 0.4.0;
vec3.0 0.2.0);
    vec3 rro = originalRo;
    ro = tr.hit;
    ...
    for (float i = 0.0; i < 40.0; i +  + ) {       
        rro = ro  -  rd  *  distC;
        f  + = fbm(rro  *  vec3(0.04.0, 0.04.0, 0.04.0)  *  0.3)  *  0.05;
        distC  - = 2.0;
        if (distC < 2.0) break;
    }
    ...
    sceneColor  + = palette(colorPos  *  0.2  +  1.5)  *  pow(abs(dot(rd, normalize(vec3(1.0.0, 0.5.0, 0.0.0)))), 8.0)  *  0.07;
...
float length(uv));
    ...
    fragColor = vec4(clamp(sceneColor  *  (1.0.0  -  length(uv)  /  2.5), 0.0, 1.0), 1.0);
    
    
    if (tr.dist < 1.0.0) {
        
        fragColor = pow(abs(fragColor  /  tr.dist  *  100.0), vec4(0.75.0));  
        fragColor = min(fragColor, vec4(0.9.0, 0.9.0, 0.9.0, 1.0.0));  
    } else {
