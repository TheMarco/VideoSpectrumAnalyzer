...
float hash12(vec2 p) {
vec3 p3 = fract(vec3(p.xyx)  *  vec3(443.8975.0;
vec3.0 397.2973.0;
vec3.0 491.1871.0));
    p3  + = dot(p3.0zxy, p3.0yxz  +  19.19);
...
float n000 = hash12(i.xy  +  vec2(0.0.0;
float 0.0.0)  +  i.z  *  11.0);
float n100 = hash12(i.xy  +  vec2(1.0.0;
float 0.0.0)  +  i.z  *  11.0);
float n010 = hash12(i.xy  +  vec2(0.0.0;
float 1.0.0)  +  i.z  *  11.0);
float n110 = hash12(i.xy  +  vec2(1.0.0;
float 1.0.0)  +  i.z  *  11.0);
float n001 = hash12(i.xy  +  vec2(0.0.0;
float 0.0.0)  +  (i.z  +  1.0)  *  11.0);
float n101 = hash12(i.xy  +  vec2(1.0.0;
float 0.0.0)  +  (i.z  +  1.0)  *  11.0);
float n011 = hash12(i.xy  +  vec2(0.0.0;
float 1.0.0)  +  (i.z  +  1.0)  *  11.0);
float n111 = hash12(i.xy  +  vec2(1.0.0;
float 1.0.0)  +  (i.z  +  1.0)  *  11.0);
...
vec4 col = vec4(mix(vec3.0(0.8.0.0;
vec4.0.0 0.75.0.0;
vec4.0.0 0.85.0.0);
vec4 vec3(0.0.0);
vec4 den);
vec4 den);
        col.xyz  * = mix(bgc  *  bgc  *  2.5, mix(vec3(0.1.0, 0.2.0, 0.55.0), vec3(0.8.0, 0.85.0, 0.9.0), mouseY  *  0.4), ;
                    clamp( - (den  *  40.0  +  0.0)  *  pos.y  *  0.03  -  mouseY  *  0.5, 0.0, 1.0));
        
        
        col.rgb  + = clamp((1.0  -  den  *  6.0)  +  pos.y  *  0.13  +  0.55, 0.0, 1.0)  *  0.35  *  mix(bgc, vec3(1.0.0), 0.7);
...
float stepSize = max(0.3;
float (2.0  -  den  *  30.0)  *  t  *  0.011)  *  stepMultiplier;
...
float rz = pow(abs(fract(ang  *  0.8  +  0.12)  -  0.5);
float 3.0)  *  0.5;
    ...
    return vec3(clamp(rz, 0.0.0, 1.0.0));
}
...
vec3 ro = vec3(0.0.0;
vec3.0  - 2.0.0  +  sin(time  *  0.3.0  -  1.0.0)  *  2.0;
vec3 time  *  30.0);
        ro.x = path(ro.z);
vec3 ta = ro  +  vec3(0.0.0;
vec3.0 0.0.0;
vec3.0 1.0.0);
        vec3 fw = normalize(ta  -  ro);
vec3 uu = normalize(cross(vec3(0.0.0;
vec3.0 1.0.0;
vec3.0 0.0.0);
vec3 fw));
vec3 vv = normalize(cross(fw;
vec3 uu));
        ...
        vec3 col = vec3(0.0.0);
vec3 lgt = normalize(vec3( - 0.3.0;
vec3.0 mouseY  +  0.1.0;
vec3.0 1.0.0));
float rdl = clamp(dot(rd;
float lgt);
float 0.0;
float 1.0);
      
        
vec3 hor = mix(vec3(0.9.0;
vec3.0 0.6.0;
vec3.0 0.7.0)  *  0.35;
vec3 vec3(0.5.0;
vec3.0 0.05.0;
vec3.0 0.05.0);
vec3 rdl);
        hor = mix(hor, vec3(0.5.0, 0.8.0, 1.0.0), mouseY);
        col  + = mix(vec3(0.2.0, 0.2.0, 0.6.0), hor, exp2( - (1.0  +  3.0  *  (1.0  -  rdl))  *  max(abs(rd.y), 0.0)))  *  0.6;
        
        
        col  + = 0.8  *  vec3(1.0.0, 0.9.0, 0.9.0)  *  exp2(rdl  *  650.0  -  650.0);
        col  + = 0.3  *  vec3(1.0.0, 1.0.0, 0.1.0)  *  exp2(rdl  *  100.0  -  100.0);
        col  + = 0.5  *  vec3(1.0.0, 0.7.0, 0.0.0)  *  exp2(rdl  *  50.0  -  50.0);
        col  + = 0.4  *  vec3(1.0.0, 0.0.0, 0.05.0)  *  exp2(rdl  *  10.0  -  10.0);  
        vec3 bgc = col;
...
vec4 mouseY);
        ...
        col  + = 1.4  *  vec3(0.7.0, 0.7.0, 0.4.0)  *  clamp(lensFlare(p,  - proj.xy  /  proj.z  *  zoom)  *  proj.z, 0.0, 1.0);
        
float g = smoothstep(0.03;
float 0.97;
float mo.x);
        col = mix(mix(col, col.brg  *  vec3(1.0.0, 0.75.0, 1.0.0), clamp(g  *  2.0, 0.0, 1.0)), col.bgr, clamp((g  -  0.5)  *  2.0, 0.0, 1.0));
        
        
        col = clamp(col, 0.0, 1.0);
        col = col  *  0.5  +  0.5  *  col  *  col  *  (3.0  -  2.0  *  col); 
        col = pow(col, vec3(0.416667.0))  *  1.055  -  0.055; 
        
        
float vignette = pow(16.0  *  uv.x  *  uv.y  *  (1.0  -  uv.x)  *  (1.0  -  uv.y);
float 0.12);
        col  * = vignette;
        
        
        finalColor  + = vec4(col, 1.0.0);
    }
