// Circular 1990s Style Spectrum Analyzer - Rectangular or Round Segments
...
// ANOTHER CHANGE: Added support for round segments (filled circles) and glow effects.
//
// [C]
...
// Segment shape and glow uniforms
uniform float uUseRoundSegments; // Whether to use round segments (0=rectangular, 1=round)
uniform float uGlowEffect;      // Whether to apply glow effect (0=off, 1=on)
uniform float uGlowRadius;      // Glow radius (0-1)
uniform float uGlowIntensity;   // Glow intensity (0-1)
...
uniform vec3 uGlowColor;        // Color for the glow effect
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Check if we're using round segments
    bool use_round_segments = uUseRoundSegments > 0.5;

    if (!use_round_segments) {
        // For rectangular segments, check if it's a border pixel
    ...
    // For round segments, we'll handle borders in the segment rendering logic
    ...
    // Check if we should use glow effect
    bool use_glow_effect = uGlowEffect > 0.5;

    // Get glow parameters with defaults if not set
    float glow_radius = (uGlowRadius > 0.0) ? uGlowRadius : 0.2;
    float glow_intensity = (uGlowIntensity > 0.0) ? uGlowIntensity : 0.5;
    vec3 glow_color = (uGlowColor.r >= 0.0 || uGlowColor.g >= 0.0 || uGlowColor.b >= 0.0) ?
                      uGlowColor : SEGMENT_COLOR;

    // Lighting logic: segment_index 0 is innermost.
    // The innermost segment (segment_index == 0) is always lit.
    bool is_segment_lit = (segment_index < lit_segments_count || segment_index == 0);

    if (use_round_segments) {
        // For round segments, we keep the original bar structure but render each segment as a circle

        // Calculate the exact center position of this segment
        float segment_radius = INNER_RADIUS + segment_start + (segment_end - segment_start) * 0.5;
        float segment_angle = (float(bar_index) + 0.5) * (2.0 * PI / float(NUM_BARS));

        // Adjust angle by the START_ANGLE_OFFSET
        segment_angle = mod(segment_angle + START_ANGLE_OFFSET, 2.0 * PI);

        // Convert to Cartesian coordinates
        vec2 segment_center = vec2(
            segment_radius * cos(segment_angle),
            segment_radius * sin(segment_angle)
        );

        // Calculate distance from current pixel to segment center
        float dist_to_center = length(centered_uv_scaled - segment_center);

        // Calculate circle radius - make it MUCH larger for visibility
        // Use a fixed size that's large enough to be clearly visible
        float circle_radius = segment_height * 0.8 * BAR_WIDTH;

        if (is_segment_lit) {
            if (dist_to_center <= circle_radius) {
                // Inside the round segment - fully lit
                fragColor = vec4(SEGMENT_COLOR, 1.0);
            } else if (use_glow_effect && dist_to_center < circle_radius + glow_radius) {
                // Inside the glow radius - calculate glow intensity
                float glow_factor = 1.0 - smoothstep(circle_radius, circle_radius + glow_radius, dist_to_center);
                glow_factor = pow(glow_factor, 1.5) * glow_intensity; // Softer glow falloff

                // Mix the glow color with the background
                vec3 final_color = mix(bg_color.rgb, glow_color, glow_factor);
                fragColor = vec4(final_color, 1.0);
            } else {
                // Outside the segment and glow - show background
                fragColor = bg_color;
            }
        } else {
            // Segment not lit - show background
            fragColor = bg_color;
        }
    } else {
        // Original rectangular segment logic
        if (is_segment_lit) {
            if (use_glow_effect) {
                // Apply glow effect to the edges of the segment
                // Calculate distance to the nearest edge
                float dist_to_edge = min(
                    min(local_horizontal_uv, 1.0 - local_horizontal_uv),
                    min(local_radial_uv, 1.0 - local_radial_uv)
                );

                // If we're inside the segment, use the segment color
                fragColor = vec4(SEGMENT_COLOR, 1.0);

                // If we're outside the segment but inside the glow radius, apply glow
                if (dist_to_edge < BORDER_SIZE && dist_to_edge > 0.0) {
                    float glow_factor = 1.0 - (dist_to_edge / BORDER_SIZE);
                    glow_factor = pow(glow_factor, 2.0) * glow_intensity;

                    // Mix the glow color with the segment color
                    vec3 final_color = mix(SEGMENT_COLOR, glow_color, glow_factor);
                    fragColor = vec4(final_color, 1.0);
                }
            } else {
                // No glow effect - just use the segment color
                fragColor = vec4(SEGMENT_COLOR, 1.0);
            }
        } else {
            // Segment not lit - show background
